...
- `bastion` (Attributes) Optional SSH bastion host used to reach this host. (see [below for nested schema](#nestedatt--hosts--ssh--bastion))
...
- `address` (String) IP address or hostname of the bastion host.
- `port` (Number) TCP port of the SSH service on the bastion host.

Optional:

- `key` (String, Sensitive) SSH private key in PEM format for the bastion host.
- `key_path` (String) Path to an SSH private key file for the bastion host.
- `user` (String) Username to log in as on the bastion host.
